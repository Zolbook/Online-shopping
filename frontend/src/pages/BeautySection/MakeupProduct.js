...
export default function MakeupProduct (){
  const [makeupProducts, setMakeupProduct] = useState([]);
    const {_id} = useParams();
    ...
    useEffect(() => {
        console.log(_id);
        axios.get(`${process.env.REACT_APP_API_URI}/makeups/${_id}`)
          .then((res) => {
            setMakeupProduct(res.data.makeup);
          })
