...
export default function MenclothProduct (){
    const {_id} = useParams();
    ...
    useEffect(() => {
        console.log(_id);
        axios.get(`${process.env.REACT_APP_API_URI}/MenCloth/${_id}`)
          .then((res) => {
            setMenProduct(res.data.mencloth);
          })
...
<div className='col-4' key={p._id} value={p._id} onClick={() => handleClick(p)} >
  ...
  <IconCheckbox
    ...
    checked={checkedArray.includes(p.id)}
